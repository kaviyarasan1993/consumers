...
public class AgreementDao {
	...
	public void saveAgreement(AgreementRequest agreementRequest) {
		...
		Object[] obj = new Object[] { agreement.getAgreementDate(), agreement.getAgreementNumber(),
		...
		for(int i=0;i<demandsCount;i++){
			Object[] demandRecord = {agreement.getTenantId(),agreement.getAgreementNumber(),demands.get(i)};
	...
	public void updateAgreement(AgreementRequest agreementRequest) {
		...
		if (agreement.getAgreementNumber() != null) {
			String sql = "UPDATE eglams_demand SET agreementnumber=" + agreement.getAgreementNumber()
					+ " WHERE demandid=?";
			List<Object[]> batchArgs = new ArrayList<>();
			List<String> demands = agreement.getDemands();
			int demandsCount = demands.size();
